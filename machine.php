<?php
	...
	if($_SESSION['role'] == 'admin')
		...
		$sqlSelect = "SELECT * FROM `websitedata` WHERE (`Username`='".$_SESSION['username']."' OR `Username`='')";
	...
	if ($mode=="download")
	{
		if($_SESSION['role'] == 'admin')
			...
			$sqlSelect_find_to_download = "SELECT * FROM `websitedata` WHERE (`Address`=\"$address\" OR `WebsiteTitle`=\"$title\") AND (`Username`='".$_SESSION['username']."' OR `Username`='') ORDER BY `LastUpdated` DESC LIMIT 1";
